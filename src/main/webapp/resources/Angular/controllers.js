...
requestModule.controller('requestCtrl', function ($scope, $http) {
    ...
    $scope.selectedItem = null;
     ...
     $scope.loadrequests = function () {
         ...
         $http({
                method: 'GET',
                url: '/OSProc/requests/get'
            }).success(function (result) {
                $scope.items = result;
        });
    }
    $scope.addRequestItem= function(){
        var itemObj = JSON.parse($scope.selectedItem); 
           
        var requestItemObj = {
                "quantity"     : $scope.quantity,
               "item"          : $scope.item
        };
        itemsL.push(requestItemObj);
    }
    
    $scope.addRequest = function () {
        // since we are getting a string in value of select box ; we now have to parse it as json again

        var requestItems =itemsL
      
        var RequestObj = {
          "requestedDate"        :  $scope.requestedDate,
          "approvedDate"        :  $scope.approvedDate,
          "declinedDate"        :  $scope.declinedDate,
          "purchasedDate"       :$scope.purchasedDate,
          "requestStatus"        :"requested",
          "requestingOrgUnit"    :$scope.requestingOrgUnit,
          "approvingOrgUnit"     :$scope.approvingOrgUnit,
          "requestItems"   :  itemsL,
          "supplierBids"    :$scope.supplierBids
        };
         console.log(itemObj);
   ...
   function clearForm () {
       $("#requestedDate").val("");
       $("#quantity").val("");
       jQuery("#itemSelect").selectedIndex = 0;
    } 
//this is to load the catagories on page load
   $(document).ready(function () {
      $scope.loadCats();
